...
int main()
{
    ...
    try {
        ...
        sm::vec<float,3> zeros = {0,0,0};
        sm::vec<float,3> ux = {1,0,0};
        sm::vec<float,3> uy = {0,1,0};
        sm::vec<float,3> uz = {0,0,1};
        auto pvp = std::make_unique<morph::PolygonVisual<>> (zeros, centroid, centroidv, sz/16.0f, 0.01f, uz, 10);
        v.bindmodel (pvp);
        pvp->finalize();
        v.addVisualModel (pvp);
        // red hex at zero
        auto pvp2 = std::make_unique<morph::PolygonVisual<>> (sm::vec<float,3>({0,0,0.01f}), zeros, uy, sz/20.0f, 0.01f, ux, 8);
        v.bindmodel (pvp2);
        pvp2->finalize();
        v.addVisualModel (pvp2);
